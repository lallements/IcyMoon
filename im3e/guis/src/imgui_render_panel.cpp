...
void ImguiRenderPanel::draw(const ICommandBuffer& rCommandBuffer)
{
    ...
    const auto viewportSize = getWindowContentRegionSize();
    m_pFramePipeline->prepareExecution(rCommandBuffer, toVkExtent2D(viewportSize), m_pRenderOutput);
    {
    ...
    ImGui::Image(reinterpret_cast<ImTextureID>(m_vkRenderOutputSet), viewportSize);
    ImGui::SetCursorScreenPos(imagePos);
    ImGui::InvisibleButton(fmt::format("{}_mouseArea", m_name).c_str(), viewportSize,
